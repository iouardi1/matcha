...
const Profile = {
    ...
    getListOfNotifs: async (email) => {
        const { rows } = await db.query(
            ...
            LEFT JOIN user_blocks b1 ON b1.blocker_id = u.id AND b1.blocked_user_id = n.senderid
            LEFT JOIN user_blocks b2 ON b2.blocker_id = n.senderid AND b2.blocked_user_id = u.id
            WHERE u.email = $1
                AND b1.id IS NULL
                AND b2.id IS NULL
